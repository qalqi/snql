...
export class TransferHandler {
  static async checkTransfer({
    event,
    block: { events, timestamp, block },
  }: SubstrateEvent) {
    const transferInfo = events.find(
      (item) => item.event.method === "Transfer"
    );
    ...
    transfer.fee = events.reduce((total, cur) => {
      ...
      if ([FeePosition[0], FeePosition[1]].includes(method)) {
        try {
          fee = BigInt(
            parseInt(
              JSON.parse(cur.event.data.toString())[
                FeePosition[cur.event.method]
              ]
            )
          );
        } catch (err) {}
